  ...
  const pegarValores = useCallback(async () => {
    try {
      ...
      if (response.status === 200) {
        const jantarData = response.data; // Use response.data para obter os dados
        setAcompanhamento1(jantarData.acompanhamento1);
        setAcompanhamento2(jantarData.acompanhamento2);
        setPratoBase(jantarData.prato_base);
        setProteina1(jantarData.proteina_1);
        setSalada(jantarData.salada);
        setSobremesa(jantarData.sobremesa);
        setVegeitariana(jantarData.opcao_vegetariana);
        setSuco(jantarData.suco);
      }
    } catch (error) {
